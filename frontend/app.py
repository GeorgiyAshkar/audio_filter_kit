...
# Attempt to import optuna for hyperparameter optimisation.  If not available,
# the application will fall back to a simple random search.  Optuna
# accelerates search by sampling intelligently and pruning poor candidates.
try:
...
class AudioFilterApp(QMainWindow):
    ...
    def _build_ui(self):
        ...
        # Use Optuna for optimisation if available; otherwise fall back to random search.
        self.btn_optimize_current = QPushButton("Оптимизация параметров (Optuna)")
        ...
        # search group
        # Use Optuna when available to search best combinations; fall back to random search.
        search_group = QGroupBox("Поиск оптимальных комбинаций (Optuna)")
    ...
    def optimize_current_pipeline(self):
        """
        Optimise the parameters of the current pipeline using Optuna if available.

        If Optuna is installed, we construct an optimisation study where each
        parameter of every filter in the pipeline is defined as a search space.
        The objective function applies the pipeline with sampled parameters to
        the raw signal and returns the negative composite score (so that
        Optuna minimises the objective).  At completion, the best parameter
        combination is applied to update the pipeline.  During optimisation,
        the status bar shows progress.  If Optuna is not available, a random
        ...
        if optuna is not None:
            # Prepare search space for each filter parameter
            # Flatten parameter names with filter class to avoid clashes
            def objective(trial):
                candidate = []
                for idx, f in enumerate(self.pipeline):
                    ...
                    spec = f.param_spec()
                    params: Dict[str, Any] = {}
                    for pname, (ptype, default, pmin, pmax, step) in spec.items():
                        key = f"{cls.__name__}.{pname}"
                        # Determine appropriate suggestion based on type
                        if ptype is bool:
                            params[pname] = trial.suggest_categorical(key, [True, False])
                        elif ptype is int:
                            if pmin is not None and pmax is not None and step:
                                # If step divides the range into a small number of values,
                                # treat as categorical to avoid sampling invalid values.
                                try:
                                    values = list(range(int(pmin), int(pmax) + 1, int(step)))
                                    if 0 < len(values) <= 50:
                                        params[pname] = trial.suggest_categorical(key, values)
                                    else:
                                        params[pname] = trial.suggest_int(key, int(pmin), int(pmax), step=int(step))
                                except Exception:
                                    params[pname] = trial.suggest_int(key, int(pmin), int(pmax))
                            elif pmin is not None and pmax is not None:
                                params[pname] = trial.suggest_int(key, int(pmin), int(pmax))
                            else:
                                # fall back to reasonable range
                                params[pname] = trial.suggest_int(key, -10, 10)
                        elif ptype is float:
                            if pmin is not None and pmax is not None and step:
                                try:
                                    n_steps = int((float(pmax) - float(pmin)) / float(step))
                                    values = [float(pmin) + i * float(step) for i in range(n_steps + 1)]
                                    if 0 < len(values) <= 50:
                                        params[pname] = trial.suggest_categorical(key, values)
                                    else:
                                        params[pname] = trial.suggest_float(key, float(pmin), float(pmax))
                                except Exception:
                                    params[pname] = trial.suggest_float(key, float(pmin), float(pmax))
                            elif pmin is not None and pmax is not None:
                                params[pname] = trial.suggest_float(key, float(pmin), float(pmax))
                            else:
                                params[pname] = trial.suggest_float(key, 0.0, 1.0)
                        elif ptype is str:
                            # String parameters cannot be sampled unless explicitly enumerated in search_space.
                            # Fall back to the current value.
                            params[pname] = default
                        else:
                            params[pname] = default
                    candidate.append(cls(**params))
                est = run_pipeline(raw, sr, candidate)
                metrics = compute_metrics(ref, est, sr)
                score = composite_score(metrics)
                # Attach the candidate pipeline and metrics to the trial for later retrieval
                trial.set_user_attr("candidate", candidate)
                trial.set_user_attr("metrics", metrics)
                trial.set_user_attr("score", score)
                # Optuna minimises the objective; use negative score
                return -score
            # Create study
            try:
                study = optuna.create_study(direction="minimize")
            except Exception:
                # Fallback: in some versions, create_study may require storage arg; create default
                study = optuna.Study(direction="minimize")
            eval_counter = {"count": 0}
            def callback(study, trial):
                eval_counter["count"] += 1
                if eval_counter["count"] % 5 == 0 or eval_counter["count"] == max_eval:
                    self.set_status(f"Optuna optimizing: {eval_counter['count']}/{max_eval} trials")
                # update best intermediate result
                best = study.best_trial
                best_candidate = best.user_attrs.get("candidate")
                best_metrics = best.user_attrs.get("metrics")
                if best_candidate is not None and best_metrics is not None:
                    self.processed_signal = run_pipeline(raw, sr, best_candidate)
                    self.show_metrics(best_metrics, sr)
                    self.redraw()
            # Run optimisation
            self.set_status("optimizing parameters with Optuna...")
            try:
                study.optimize(objective, n_trials=max_eval, callbacks=[callback])
            except Exception as e:
                # If any error occurs, fallback to random search below
                QMessageBox.warning(self, "Оптимизация прервана", f"Ошибка Optuna: {str(e)}. Используется случайный поиск.")
                study = None
            if study is not None:
                best_trial = study.best_trial
                best_candidate = best_trial.user_attrs.get("candidate")
                best_metrics = best_trial.user_attrs.get("metrics")
                if best_candidate is not None:
                    self.pipeline = [f.clone() for f in best_candidate]
                    self.refresh_pipeline_list()
                    self.refresh_param_editor(self.pipeline_list.currentRow())
                    ...
                    if best_metrics is not None:
                        self.show_metrics(best_metrics, sr)
                    self.redraw()
                QMessageBox.information(self, "Готово", f"Оптимизация завершена. Проверено {min(max_eval, eval_counter['count'])} вариантов.")
                self.set_status("готово")
                return
        # Fallback: random search over search_space
        search_spaces = [f.search_space() for f in self.pipeline]
        if not search_spaces or any(len(s) == 0 for s in search_spaces):
            QMessageBox.warning(self, "Нет пространства поиска", "Не удалось построить пространство поиска для фильтров.")
            return
        best_score = -1e18
        best_candidate = None
        best_metrics = None
        self.set_status("optimizing parameters (random search)...")
        for i in range(1, max_eval + 1):
            combo = [random.choice(s) for s in search_spaces]
            candidate = [f.__class__(**params) for f, params in zip(self.pipeline, combo)]
            est = run_pipeline(raw, sr, candidate)
            metrics = compute_metrics(ref, est, sr)
            score = composite_score(metrics)
            if score > best_score:
            ...
            if i % 5 == 0 or i == max_eval:
                self.set_status(f"Random optimizing: {i}/{max_eval}")
                self.redraw()
        if best_candidate is not None:
            self.pipeline = [f.clone() for f in best_candidate]
            self.refresh_pipeline_list()
            ...
            if best_metrics is not None:
                self.show_metrics(best_metrics, sr)
            self.redraw()
        QMessageBox.information(self, "Готово", f"Случайная оптимизация завершена. Проверено {max_eval} вариантов.")
        self.set_status("готово")
    ...
    def search_best_pipelines(self):
        """
        Search for the best combinations of filters and parameters using Optuna if available.

        This method considers varying pipeline lengths up to `max_depth`.  For each trial,
        a random (or Optuna-guided) sequence of filters is selected from the list of
        available classes, and parameters are sampled according to each filter's
        search space.  Metrics are computed against the reference signal, and the
        pipelines are ranked by the composite score.  The top‑k results are shown
        in the UI.  If Optuna is unavailable, a random search fallback is used.
        """
        try:
            raw, ref, sr = self.get_eval_pair()
        except Exception as e:
        ...
        if optuna is not None:
            ...
            # Map class to its parameter spec function
            def objective(trial):
                # sample length of pipeline
                depth = trial.suggest_int("n_filters", 1, max_depth)
                pipeline: List[FilterBase] = []
                # Build pipeline
                for i in range(depth):
                    cls = trial.suggest_categorical(f"filter_{i}_class", class_options)
                    # Sample parameters via search space or param_spec
                    params: Dict[str, Any] = {}
                    # Try to get search_space list for this class
                    # Instead of using discrete search_space, we use param_spec for continuous suggestions
                    spec = cls().param_spec()
                    for pname, (ptype, default, pmin, pmax, step) in spec.items():
                        key = f"f{i}.{cls.__name__}.{pname}"
                        if ptype is bool:
                            params[pname] = trial.suggest_categorical(key, [True, False])
                        elif ptype is int:
                            if pmin is not None and pmax is not None and step:
                                try:
                                    vals = list(range(int(pmin), int(pmax) + 1, int(step)))
                                    if 0 < len(vals) <= 50:
                                        params[pname] = trial.suggest_categorical(key, vals)
                                    else:
                                        params[pname] = trial.suggest_int(key, int(pmin), int(pmax), step=int(step))
                                except Exception:
                                    params[pname] = trial.suggest_int(key, int(pmin), int(pmax))
                            ...
                            if pmin is not None and pmax is not None and step:
                                try:
                                    n_steps = int((float(pmax) - float(pmin)) / float(step))
                                    vals = [float(pmin) + j * float(step) for j in range(n_steps + 1)]
                                    if 0 < len(vals) <= 50:
                                        params[pname] = trial.suggest_categorical(key, vals)
                                    else:
                                        params[pname] = trial.suggest_float(key, float(pmin), float(pmax))
            ...
            try:
                ...
                QMessageBox.warning(self, "Ошибка", f"Optuna search failed: {str(e)}. Переходим к случайному поиску.")
                study = None
            if study is not None:
                ...
                for t in trials[:top_k]:
                    ...
                    if pipe is None or metrics is None:
                        continue
                    desc = " -> ".join(f.name for f in pipe)
                    results.append({
                        "pipeline": pipe,
                        "metrics": metrics,
                        "score": score,
                ...
                for idx, res in enumerate(self.search_results, start=1):
                    ...
                    self.results_list.addItem(
                        f"{idx}. {res['desc']} | SNR={m['snr']:.2f} | SegSNR={m['seg_snr']:.2f} | LSD={m['lsd']:.2f} | MSE={m['mse']:.6f}"
                    )
        ...
        self.set_status("searching best pipelines (random search)...")
        best_pipelines: List[Dict[str, Any]] = []
        for i in range(1, max_eval + 1):
            depth = random.randint(1, max_depth)
            pipeline: List[FilterBase] = []
            for _ in range(depth):
                cls = random.choice(classes)
                # pick random parameter combination from search_space
                space = cls().search_space()
                if space:
                    params = random.choice(space)
                    pipeline.append(cls(**params))
                else:
                    pipeline.append(cls())
            metrics, score = evaluate_candidate(pipeline)
    ...
    def show_help(self):
        text = (
            ...
            "<b>Оптимизация параметров:</b> ищет лучшие параметры для текущей последовательности фильтров с помощью Optuna (при наличии) или случайного поиска, выбирая комбинацию с наилучшей метрикой.<br>"
            "<b>Поиск оптимальных комбинаций:</b> использует Optuna (при наличии) для подбора цепочек фильтров и их параметров. При отсутствии Optuna запускается случайный поиск по доступным фильтрам и их параметрам.<br>"
